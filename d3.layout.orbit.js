d3.layout.orbit = function() {
	...
	_orbitLayout.mode = function(_mode) {
		...
		if (_mode == "solar") {
			fixedOrbitArray = [strongStrongCount, strongModCount, strongWeakCount, midCircleCount, outerCircleCount]
			//fixedOrbitArray = [strongStrongCount, strongModCount, strongWeakCount, modWeakCount, modModCount, modStrongCount, weakWeakCount, weakModCount, weakStrongCount]
		}
		if (_mode == "atomic") {
			fixedOrbitArray = [1, 1, 1, 1, 1, 1, 1, 1, 1]
		}
		if (_mode == "flat") {
			fixedOrbitArray = [99]
		}
			...
			if(childrenAccessor(_node)) {
				...
				if (fixedOrbitArray[_currentRing-1]) {
					...
					for (var x = y; x<y+ringSize && x<totalChildren;x++) {
						childrenAccessor(_node)[x].angle = ((piedValues[x - y].endAngle - piedValues[x - y].startAngle) / 2) + piedValues[x - y].startAngle * 0.5;

						childrenAccessor(_node)[x].parent = _node;
						childrenAccessor(_node)[x].depth = _node.depth + 1;

						childrenAccessor(_node)[x].x = (childrenAccessor(_node)[x].parent.x + ( (childrenAccessor(_node)[x].parent.ring / 4) * Math.sin( childrenAccessor(_node)[x].angle ) )) * 0.3;
						childrenAccessor(_node)[x].y = childrenAccessor(_node)[x].parent.y + ( (childrenAccessor(_node)[x].parent.ring / 4) * Math.cos( childrenAccessor(_node)[x].angle ) );
